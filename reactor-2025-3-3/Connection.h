...
//这里使用std::enable_shared_from_this<Connection>是为了在Connection类中能够使用shared_from_this()函数，返回一个指向当前对象的shared_ptr指针
//因为在类中的回调函数中，原+本我们传递的是this指针，而this指针是一个普通指针，通过这种方式，调用shared_from_this()函数，返回一个指向当前对象的shared_ptr指针，这样就可以保证对象的生命周期
{
private:
    EventLoop* loop_;   //对应的事件循环，但是这个事件循环是从外面传入的，不属于这个类的对象
...
public:
    ...
    string ip()const;
    uint16_t port();
    void close_client();          //TCP连接关闭(断开)的回调函数,供Channel回调
    void error_client();          //TCP连接错误的回调函数,供Channel回调
