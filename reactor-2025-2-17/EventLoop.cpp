...
EventLoop::EventLoop(bool mainloop,int time_tvl,int time_out_):stop_(false),ep_(new Epoll),wakeupfd_(eventfd(0,EFD_NONBLOCK)),wakeup_channel(new Channel(this,wakeupfd_)),
...
{
    stop_=true;
    wakeup();//因为事件循环函数run阻塞在ep_->loop()函数中，除非闹钟响了或者有事件发生epoll_wait才会立即返回，所有调用wakeup函数立即唤醒事件循环
}
